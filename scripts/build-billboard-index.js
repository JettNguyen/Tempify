#!/usr/bin/env node
/**
 * Processes src/assets/hot-100-current.csv into a compact lookup index.
 * Output: src/assets/hot-100-index.json
 *
 * Run: node scripts/build-billboard-index.js
 *
 * Index format:
 *   { "normalized title": [["normalized artist", peakPos, weeksAtOne], ...] }
 *
 * Multiple entries per title handle songs recorded by different artists.
 */
...
const CSV  = path.join(__dirname, '../src/assets/hot-100-current.csv')
const OUT  = path.join(__dirname, '../src/assets/hot-100-index.json')
...
rl.on('close', () => {
  // Restructure: title → [[artist, peak, weeksAtOne], ...]
  const index = {}
  ...
  // Sort each title's entries by peak position (best first)
  ...
  console.log(`✓ Built billboard index: ${titles} titles, ${entries} title+artist entries → ${OUT}`)
})
